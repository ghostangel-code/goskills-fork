...
func GenerateToolDefinitions(skill *SkillPackage) ([]openai.Tool, map[string]string) {
	...
	if hasCustomTools {
		...
		// 【方案1改造】没有自定义工具，也没有脚本文件
		// 这是纯文档型 SKILL.md（如 weather-query）
		// 不生成任何工具定义，让 LLM 直接生成代码
		// 只保留 bash 工具用于执行内联命令
		for _, t := range baseTools {
...
// generateScriptTool 生成脚本工具
func generateScriptTool(skillPath string, scriptRelPath string) (openai.Tool, string) {
